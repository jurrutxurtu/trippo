...
| `OVERNIGHT_MIN_HOURS` | 5.0 | Alternative trigger for a long dusk→dawn stay that misses the core window. |
...
| Constant | Value | Rationale |
|---|---|---|
| `MAX_GROUND_SPEED_KMH` | 130 | Motorway speed plus margin. A "stationary" visit that requires more than this to reach the next fix was not stationary — it was a crossing. Deliberately generous: false negatives are safer than deleting a real stop. |
...
Either condition triggers an `unknown` event. Never auto-classified — see ADR-0007.
...
| Constant | Value | Rationale |
|---|---|---|
| `GPX_ABSORB_OVERLAP` | 0.5 | ≥50% temporal overlap with a timeline segment ⇒ the track absorbs it; otherwise the track becomes a standalone activity. |
| `GPX_SIMPLIFY_TOLERANCE_M` | 8 | Douglas-Peucker tolerance. ~3,200 points → ~1,200 with no visible change at z14. |
